...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 30 11:56:45 2024
...
lambda_ = np.logspace(-3,3,20)[ int(eval(sys.argv[4]))]
...
if not Path(os.path.join(outdir, f'DA_{method}_{ int(eval(sys.argv[4]))}.csv')).is_file():
    remove_noise=True
    ...
    results.to_csv(os.path.join(outdir, f'DA_{method}_{ int(eval(sys.argv[4]))}.csv'))
