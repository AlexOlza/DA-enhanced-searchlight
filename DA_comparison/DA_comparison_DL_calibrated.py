...
result_filename = os.path.join(outdir, f'DA_DL_{method}_reduce{REDUCE_DIM_NAME}_calibrated.csv')
...
if True:#not Path(result_filename).is_file():
    ...
    for Nt in tqdm(Nts):
        ...
        prediction_fname =os.path.join(outdir, f'DL_{method}_preds_{Nt}_reduce{REDUCE_DIM_NAME}_calibrated')
